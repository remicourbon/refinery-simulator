...
def get_prix_produits_eia() -> dict:
    """
    Prix spot actuels des produits.
    EIA pour Diesel et Kerosene.
    Différentiels fixes sur le Brent pour les autres.
    """
...
def get_prix_bruts() -> dict:
    ...
    Urals, Arab Light et Sahara calculés par différentiel sur le Brent.
    """
    brent = get_prix_brent()
    return {
        "Brent":        round(brent,        2),
...
def get_historique_brent(jours: int = 365) -> pd.DataFrame:
    """Historique du prix Brent sur N jours."""
...
def get_historique_produit(produit: str, jours: int = 365) -> pd.DataFrame:
    ...
    Disponible uniquement pour Diesel et Kerosene.
    """
    codes = {
    ...
    if produit not in codes:
        raise ValueError(f"{produit} non disponible sur EIA")
